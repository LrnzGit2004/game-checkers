...
class Board {
  ...
  createCell(row, col, boardElement) {
    ...
    cell.className = `case ${
      (row + col) % 2 === 0 ? "case-claire" : "case-foncee"
    }`;
  ...
  clearHighlights() {
    document
      .querySelectorAll(".case-possible")
  ...
  movePiece(move) {
    ...
    if (move.capture) {
      ...
      document
        .querySelector(
          `[data-row="${move.capture.row}"][data-col="${move.capture.col}"] .pion`
        )
        .remove();
    }
  ...
  checkForKing(piece) {
    if (
      ...
      document
        .querySelector(
          `[data-row="${piece.row}"][data-col="${piece.col}"] .pion`
        )
        .classList.add("dame");
    }
  }
